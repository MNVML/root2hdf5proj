        ...
        double valid_test_threshold)
{
    ...
    */
    ...
    auto leptodat_train = make_ntuple({hdffile_train, "lepto_data"},
    ...
    auto leptodat_valid = make_ntuple({hdffile_valid, "lepto_data"},
            make_scalar_column<uint32_t>("n_electrons"),
            make_scalar_column<uint32_t>("n_muons"),
            make_scalar_column<uint32_t>("n_taus"),
            make_scalar_column<float>("esum_electrons"),
    ...
    auto leptodat_test = make_ntuple({hdffile_test, "lepto_data"},
            make_scalar_column<uint32_t>("n_electrons"),
            make_scalar_column<uint32_t>("n_muons"),
            make_scalar_column<uint32_t>("n_taus"),
            make_scalar_column<float>("esum_electrons"),
    ...
    for (i = first_event_number; i < n_proc; ++i) {
        ...
        if (train_valid_test_score < train_threshold) {
            ...
            if (!is_data) {
                vtxdat_train.insert(segment, planecode, true_z);
                gendat_train.insert(current, int_type, targetZ, W, x_bj, y_bj, Q2);
            ...
            if (!is_data) {
                vtxdat_valid.insert(segment, planecode, true_z);
                gendat_valid.insert(current, int_type, targetZ, W, x_bj, y_bj, Q2);
            ...
            if (!is_data) {
                vtxdat_test.insert(segment, planecode, true_z);
                gendat_test.insert(current, int_type, targetZ, W, x_bj, y_bj, Q2);
